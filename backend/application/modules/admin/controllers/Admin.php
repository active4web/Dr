...
$week_d=date_format($date,"d");
 ...
 $this->data = array( 
 'total_visitor_day'=> $this->db->get_where('visitings',array('day_t'=>$day_d,'month_d'=>$month_d,'year_d'=>$year_d))->result(),
  'total_visitor_month'=> $this->db->get_where('visitings',array('month_d'=>$month_d,'year_d'=>$year_d))->result(),
 'total_final'=> $this->db->get_where('visitings')->result());
...
public function statistics(){
  $this->load->view('admin/setting/statistics');
  }
  ...
  $this->load->view('admin/admin/logout');
      ...
      if (count((array)$findemail)>0)
        {
          ...
          $result = $this->email
        ->from('info@p2.wisyst.info')
        ->reply_to('info@p2.wisyst.info')    // Optional, an account where a human being reads.
        ->to($email)
        ->subject($subject)
        ->message($body)
        ->send();
    ...
    public function submit_login(){
      ...
      $customer_id = $this->data->get_table_row('admin',array('username'=>$username,'password'=>$passwordp,'view'=>'1'),'id');
          if($customer_id != ""){
          $site_name = $this->data->get_table_row('site_infos',array(),'name_site');
					$site_favicon = $this->data->get_table_row('site_infos',array(),'favicon');
...
$job_link=$this->input->post('job_link');
...
$about_footer_en=$this->input->post('about_footer_en');
...
if($_FILES['file1']['name']!=""){
  ...
  $file_name="file1";
  $config=get_img_config('site_infos','uploads/site_setting/',$file,$file_name,'favicon','gif|jpg|png|jpeg',600000,600000,600000,array('id'=>1),"32","32");
  if($config!=0){
       $site_favicon = $this->data->get_table_row('site_infos',array(),'favicon');
       $this->session->set_userdata(array('site_favicon' => $site_favicon));
  }
...
if($_FILES['logo_en']['name']!=""){
  $file=$_FILES['logo_en']['name'];
  $file_name="logo_en";
  $config=get_img_config('site_infos','uploads/site_setting/',$file,$file_name,'logo_en','gif|jpg|png|jpeg',600000,600000,600000,array('id'=>1),"240","50");
   }

if($_FILES['footer_log_en']['name']!=""){
  $file=$_FILES['footer_log_en']['name'];
  $file_name="footer_log_en";
  $config=get_img_config('site_infos','uploads/site_setting/',$file,$file_name,'footer_log_en','gif|jpg|png|jpeg',600000,600000,600000,array('id'=>1),"190","50");
   }

if($_FILES['footer_log']['name']!=""){
  $file=$_FILES['footer_log']['name'];
  $file_name="footer_log";
  $config=get_img_config('site_infos','uploads/site_setting/',$file,$file_name,'footer_log','pdf|doc',120000000,120000000,120000000,array('id'=>1),"190","50");
}
...
if($_FILES['pdf_intro']['name']!=""){
  ...
  $file_name="pdf_intro";
  $config=get_img_config('site_infos','uploads/site_setting/',$file,$file_name,'intor_pdf','pdf|doc',120000000,120000000,120000000,array('id'=>1));
...
/***************** START SLIDER **********************/
  ...
  $pg_config['sql'] = $this->data->get_sql('sliders','','id','DESC');
  $pg_config['per_page'] =15;
  $data = $this->lib_pagination->create_pagination($pg_config);
...
redirect(base_url().'admin/slider_home','refresh');
  ...
  $id = $this->input->post("id");
  $ser = $this->db->get_where("sliders",array("id"=>$id,"view" => "1"))->num_rows();
  if ($ser == 1) {
  $this->db->update("sliders",array("view" => "0"),array("id"=>$id));
  echo "0";
  }
  if ($ser == 0) {
  $this->db->update("sliders",array("view" => "1"),array("id"=>$id));
  echo "1";
        }      
    } 
...
$check=$this->input->post('check');
if($product_id!=""){
$img = get_this('sliders',['id' => $product_id],'img');
if ($img != "") {
unlink("uploads/slider/$img");
}
$ret_value=$this->data->delete_table_row('sliders',array('id'=>$product_id));
}
if(isset($check)&&$check!=""){  
  ...
  $length=count($check);
  for($i=0;$i<$length;$i++){
      $img = get_this('sliders',['id' => $check[$i]],'img');
if ($img != "") {
unlink("uploads/slider/$img");
}
$ret_value=$this->data->delete_table_row('sliders',array('id'=>$check[$i]));     
 }
 }
 $this->load->helper('url');
...
'num_admin'=> $this->data->get_table_data('admin'),
'site_info'=> $this->data->get_table_data('site_infos'),
'silder_data'=> $this->data->get_table_data('sliders',array('id'=>$id_slider)));
...
$img = get_this('sliders',['id' => $id],'img');
if ($img != "") {
unlink("../uploads/slider/$img");
}
