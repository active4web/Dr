...
public function __construct(){
    ...
    public function index(){
        redirect(base_url().'admin/reservation/reservation_data','refresh');
    }



    public function reservation_data(){
      $pg_config['sql'] = $this->data->get_sql('reservations','','id_message','DESC');
      $pg_config['per_page'] =20;
      $data = $this->lib_pagination->create_pagination($pg_config);
      $this->load->view("admin/reservation/messages", $data); 
    }
  public function banner(){
    ...
    public function messages_jobs(){
      $pg_config['sql'] = $this->data->get_sql('home_messages','','id_message','DESC');
      $pg_config['per_page'] =20;
      $data = $this->lib_pagination->create_pagination($pg_config);
    ...
    public function check_view_message(){
      $id = $this->input->post("id");
      $ser = $this->db->get_where("reservations",array("id_message"=>$id,"view" => "1"))->num_rows();
      if ($ser == 1) {
      $this->db->update("reservations",array("view" => "0"),array("id_message"=>$id));
      echo "0";
      }
      if ($ser == 0) {
      $this->db->update("reservations",array("view" => "1"),array("id_message"=>$id));
      echo "1";
            }      
        } 
        ...
        public function check_view_jobmessage(){
          $id = $this->input->post("id");
          $ser = $this->db->get_where("home_messages",array("id_message"=>$id,"view" => "1"))->num_rows();
          if ($ser == 1) {
          $this->db->update("home_messages",array("view" => "0"),array("id_message"=>$id));
          echo "0";
          }
          if ($ser == 0) {
          $this->db->update("home_messages",array("view" => "1"),array("id_message"=>$id));
          echo "1";
                }      
            } 
        ...
        public function delete_message(){
          ...
          $check=$this->input->post('check');
          if($product_id!=""){
          $ret_value=$this->data->delete_table_row('reservations',array('id_message'=>$product_id));
          }
          if(isset($check)&&$check!=""){  
            $check=$this->input->post('check');
            $length=count($check);
            for($i=0;$i<$length;$i++){
          $ret_value=$this->data->delete_table_row('reservations',array('id_message'=>$check[$i]));     
           }
           }
           $this->load->helper('url');
           $this->session->set_flashdata('msg', 'تم الحذف بنجاح');
           $this->session->mark_as_flash('msg');
           redirect('/admin/reservation', 'refresh');
            }
            ...
            public function delete_jobmessage(){
              ...
              $check=$this->input->post('check');
              if($product_id!=""){
              $ret_value=$this->data->delete_table_row('home_messages',array('id_message'=>$product_id));
              }
              if(isset($check)&&$check!=""){  
                $check=$this->input->post('check');
                $length=count($check);
                for($i=0;$i<$length;$i++){
              $ret_value=$this->data->delete_table_row('home_messages',array('id_message'=>$check[$i]));     
               }
               }
               $this->load->helper('url');
               $this->session->set_flashdata('msg', 'تم الحذف بنجاح');
            ...
            $id_slider=$this->input->get('id');
              $this->db->update("reservations",array("view" => "1"),array("id_message"=>$id_slider));
          $this->data = array(
          'messages_data'=> $this->data->get_table_data('reservations',array('id_message'=>$id_slider)));
          $this->load->view('admin/reservation/view_message',$this->data);
            } 
            ...
            Public function view_jobmessage(){
              $id_slider=$this->input->get('id');
            $this->db->update("home_messages",array("view" => "1"),array("id_message"=>$id_slider));
            $this->data = array(
            'messages_data'=> $this->data->get_table_data('home_messages',array('id_message'=>$id_slider)));
                 ...
                 <td align='center' valign='top' width='100%'' style='background-color: #f7f7f7; height: 100px;'>
                   ...
                   <table cellspacing='0' cellpadding='0' width='600' class='w320'>
                     ...
                     <td style='padding:5px 0 25px' class='free-text headerlgb1'>
                       &copy; جميع الحقوق محفوظة لدى شركة <a href='https://mada-sa.com'>SOAPCO-السوابوكى   </a>  <br />
                      <br /><br />
                     </td>
                     </tr>
                   </table>
